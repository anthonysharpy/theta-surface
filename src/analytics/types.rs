...
impl SVICurveParameters {
    ...
    pub fn check_valid(params: &SVICurveParameters) -> Result<(), UnsolveableError> {
        ...
        if params.b * (1.0 + params.p) <= 0.0 {
            return Err(UnsolveableError::new("Asymptotic slope of total variance must be greater than 0 (a)"));
        }
        if params.b * (1.0 + params.p) >= 2.0 {
            return Err(UnsolveableError::new("Asymptotic slope of total variance must be less than 2 (a)"));
        }
        if params.b * (1.0 - params.p) <= 0.0 {
            return Err(UnsolveableError::new("Asymptotic slope of total variance must be greater than 0 (b)"));
        }
        if params.b * (1.0 - params.p) >= 2.0 {
            return Err(UnsolveableError::new("Asymptotic slope of total variance must be less than 2 (b)"));
        }
