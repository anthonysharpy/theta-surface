...
- When using the smile graph, we determine a forward price for the underlying (Bitcoin). We do this by taking the put/call pair that has the least uncertain forward-adjusted bid/ask spread and then calculating the forward price from that, using the same forward-price calculation as we use for solving implied volatility.
- Creating the smile graph ("fitting") involves using a guessing-based algorithm to find the most accurate curve that fits the data. In this case the Levenberg-Marquardt algorithm is used. The curve we fit is based on the SVI formula, which is designed so as to not produce curves that are impossible (according to conventional enonomic theory). In order to arrive at the best-fit, we also brute force starting guesses.
